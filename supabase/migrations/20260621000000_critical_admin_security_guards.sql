-- Critical guardrails: blocked-account RLS, role-aligned admin deletes,
-- private staff-chat attachments, protected profile fields, and atomic profile merge.
...
as $$
  select not exists (
    ...
    where p.id = auth.uid()
      and (
        lower(coalesce(p.account_status, '')) in ('banned', 'suspended')
        or (
...
as $$
  select case
    when p.is_admin is true
      and lower(coalesce(p.account_status, '')) not in ('banned', 'suspended')
      and (
