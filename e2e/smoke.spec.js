...
test.describe('Smoke Chez Moi CI', () => {
  ...
  test('navigation categorie annonces hydrate les filtres URL', async ({ page }) => {
    await page.goto(`${base}/annonces?type=location`)
    await expect(page.getByRole('heading', { level: 1, name: /logements à louer/i })).toBeVisible()

    await page.getByRole('link', { name: /services & pros/i }).first().click()
    await expect(page).toHaveURL(/\/annonces\?type=prestations/)
    await expect(page.getByRole('heading', { level: 1, name: /prestataires & artisans/i })).toBeVisible()
    ...
    await expect(page.getByRole('heading', { level: 1, name: /biens à vendre/i })).toBeVisible()
  })
