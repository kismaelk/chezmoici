...
function staffChatAttachmentPath(storedValue) {
...
async function mapStaffDiscussionRow(m, reactionRows, currentUserId) {
  const reactions = {}
  const my_reactions = []
  ...
  return {
    ...
    body: m.body,
    attachment_url: await signedStaffChatAttachmentUrl(m.attachment_url),
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all((data || []).map((m) =>
