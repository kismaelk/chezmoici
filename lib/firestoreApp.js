...
export async function addNotification(data) {
  ...
  if (payload.lien != null) {
    payload.lien = safeNotificationHref(payload.lien)
  }
...
export async function uploadStaffChatAttachment(file) {
  ...
  if (error) throw error
  const { data } = supabase.storage.from('staff-chat').getPublicUrl(path)
  return { url: data.publicUrl, name: safeName, mime: file.type || 'application/octet-stream' }
}
