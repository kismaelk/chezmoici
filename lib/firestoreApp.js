...
function staffChatAttachmentPath(value) {
  ...
  const markers = [
    ...
    '/storage/v1/object/sign/staff-chat/',
  ]
  ...
  if (raw.startsWith('staff-chat/')) return raw.slice('staff-chat/'.length)
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rowsWithSignedAttachments = await Promise.all(
    (data || []).map(async (m) => ({
      ...m,
      attachment_url: await signedStaffChatAttachmentUrl(m.attachment_url),
    }))
  )
  const rows = rowsWithSignedAttachments.map((m) =>
