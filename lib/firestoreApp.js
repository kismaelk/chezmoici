...
import {
  ...
  STAFF_CHAT_BUCKET,
  STAFF_CHAT_SIGNED_URL_EXPIRES_IN,
...
function mapStaffDiscussionRow(m, reactionRows, currentUserId, signedAttachmentUrl = null) {
  const reactions = {}
  const my_reactions = []
...
async function createStaffChatAttachmentSignedUrl(storedValue) {
  const path = normalizeStaffChatAttachmentPath(storedValue)
  if (!path) return null
  const { data, error } = await supabase.storage
    .from(STAFF_CHAT_BUCKET)
    .createSignedUrl(path, STAFF_CHAT_SIGNED_URL_EXPIRES_IN)
  if (error) {
    console.warn('[staff-chat] signed URL unavailable', error)
    return null
  }
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all((data || []).map(async (m) =>
    mapStaffDiscussionRow(
      ...
      reactionRows,
      user?.id,
      await createStaffChatAttachmentSignedUrl(m.attachment_url)
    )
  ))
  rows.reverse()
  return rows
...
export async function insertStaffDiscussionMessage(body, attachment = null) {
  ...
  const attachmentPath =
    attachment?.path || normalizeStaffChatAttachmentPath(attachment?.url)
