...
import {
  createStaffChatAttachmentSignedUrl,
  STAFF_CHAT_BUCKET,
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all((data || []).map(async (m) => {
    ...
    if (m.attachment_url) {
      try {
        attachmentUrl = await createStaffChatAttachmentSignedUrl(supabase, m.attachment_url)
      } catch (e) {
        console.warn('[staff-chat] signed attachment URL failed', e)
      }
    }
    return mapStaffDiscussionRow({ ...m, attachment_url: attachmentUrl }, reactionRows, user?.id)
  }))
  rows.reverse()
  return rows
}
...
/** Pièce jointe chat staff (bucket privé `staff-chat`). */
