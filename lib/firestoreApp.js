...
export async function fetchStaffDiscussionMessages(limit = 200) {
  const { data, error } = await supabase
    ...
    .select('id, author_id, body, created_at, profiles(nom, admin_role, is_admin)')
    .order('created_at', { ascending: false })
    .limit(limit)
  if (error) throw error
...
export async function insertStaffDiscussionMessage(body) {
...
export function listenStaffDiscussionMessages(onUpdate) {
  const channel = supabase
    ...
    .on(
      'postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'staff_discussion_messages' },
      () => {
