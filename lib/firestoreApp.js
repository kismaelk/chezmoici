...
const STAFF_CHAT_BUCKET = 'staff-chat'
const STAFF_CHAT_SIGNED_URL_TTL_SECONDS = 60 * 60
...
function staffChatAttachmentPath(value) {
  const raw = String(value || '').trim()
  if (!raw) return null
  const withoutQuery = raw.split('?')[0]
  const bucketPrefix = `${STAFF_CHAT_BUCKET}/`
  if (withoutQuery.startsWith(bucketPrefix)) {
    return withoutQuery.slice(bucketPrefix.length).replace(/^\/+/, '') || null
  }
  ...
  try {
    ...
    return decodeURIComponent(url.pathname.slice(url.pathname.indexOf(marker) + marker.length)) || null
  } catch {
    return null
  }
}

async function signedStaffChatAttachmentUrl(storedValue) {
  const path = staffChatAttachmentPath(storedValue)
  if (!path) return null
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all(
    (data || []).map(async (m) =>
      mapStaffDiscussionRow(
        {
          ...m,
          attachment_url: m.is_deleted ? null : await signedStaffChatAttachmentUrl(m.attachment_url),
        },
        reactionRows,
        user?.id
      )
...
/** Pièce jointe chat staff (bucket privé `staff-chat`, liens signés au chargement). */
