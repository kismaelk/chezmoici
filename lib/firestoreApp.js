...
function staffChatAttachmentPath(value) {
  ...
  try {
    ...
    const publicPrefix = '/storage/v1/object/public/staff-chat/'
    const signedPrefix = '/storage/v1/object/sign/staff-chat/'
    ...
    return raw.replace(/^staff-chat\//, '')
  }
}

async function signStaffChatAttachment(value) {
  const path = staffChatAttachmentPath(value)
  if (!path) return null
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const signedAttachmentUrls = await Promise.all(
    (data || []).map((m) => signStaffChatAttachment(m.attachment_url))
  )
  const rows = (data || []).map((m, index) =>
    mapStaffDiscussionRow(
      { ...m, attachment_url: signedAttachmentUrls[index] },
      reactionRows,
      user?.id
    )
  )
...
export async function insertStaffDiscussionMessage(body, attachment = null) {
  ...
  const attachmentRef = attachment?.path || attachment?.url || null
  if (!trimmed && !attachmentRef) throw new Error('Message vide')
...
export async function uploadStaffChatAttachment(file) {
  ...
  if (error) throw error
  return { path, url: path, name: safeName, mime: file.type || 'application/octet-stream' }
}
