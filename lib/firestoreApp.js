...
async function signedStaffChatAttachmentUrl(value) {
...
async function signStaffDiscussionAttachments(rows) {
  return Promise.all(
    rows.map(async (row) => {
      if (!row.attachment_url) return row
      return {
        ...row,
        attachment_url: await signedStaffChatAttachmentUrl(row.attachment_url),
      }
    })
  )
}
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await signStaffDiscussionAttachments(
    (data || []).map((m) => mapStaffDiscussionRow(m, reactionRows, user?.id))
  )
  rows.reverse()
  return rows
}
