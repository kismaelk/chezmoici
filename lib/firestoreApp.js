...
const STAFF_CHAT_SIGNED_URL_EXPIRES_IN = 60 * 60

async function getStaffChatAttachmentSignedUrl(storedValue) {
  ...
  const rows = await Promise.all(
    (data || []).map(async (m) =>
      mapStaffDiscussionRow(
        {
          ...m,
          attachment_url: await getStaffChatAttachmentSignedUrl(m.attachment_url),
        },
        reactionRows,
        user?.id
      )
...
export async function insertStaffDiscussionMessage(body, attachment = null) {
  ...
  const attachmentPath = normalizeStaffChatAttachmentPath(attachment?.path ?? attachment?.url)
