...
const STAFF_CHAT_SIGNED_URL_TTL_SECONDS = 30 * 60
...
async function createStaffChatAttachmentUrl(value) {
...
function mapStaffDiscussionRow(m, reactionRows, currentUserId, attachmentUrl) {
  const reactions = {}
  const my_reactions = []
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all(
    (data || []).map(async (m) =>
      mapStaffDiscussionRow(m, reactionRows, user?.id, await createStaffChatAttachmentUrl(m.attachment_url))
    )
  )
  rows.reverse()
  return rows
