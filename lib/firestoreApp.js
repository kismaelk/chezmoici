...
import {
  STAFF_CHAT_BUCKET,
  STAFF_CHAT_SIGNED_URL_TTL_SECONDS,
  extraireCheminPieceJointeStaffChat,
} from '@/lib/staffChatAttachments'
...
async function signerPieceJointeStaffChat(value) {
  const path = extraireCheminPieceJointeStaffChat(value)
  if (!path) return { path: null, signedUrl: null }
  const { data, error } = await supabase.storage
    .from(STAFF_CHAT_BUCKET)
    .createSignedUrl(path, STAFF_CHAT_SIGNED_URL_TTL_SECONDS)
  if (error) throw error
...
function mapStaffDiscussionRow(m, reactionRows, currentUserId) {
  ...
  return {
    ...
    attachment_path: extraireCheminPieceJointeStaffChat(m.attachment_url),
    reactions,
    my_reactions,
  }
}
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all((data || []).map(async (m) => {
    const row = mapStaffDiscussionRow(m, reactionRows, user?.id)
    if (row.attachment_path) {
      const { signedUrl } = await signerPieceJointeStaffChat(row.attachment_path)
      row.attachment_url = signedUrl
    } else {
...
export async function insertStaffDiscussionMessage(body, attachment = null) {
  ...
  const attachmentPath = attachment?.path || extraireCheminPieceJointeStaffChat(attachment?.url)
  if (!trimmed && !attachmentPath) throw new Error('Message vide')
...
export async function uploadStaffChatAttachment(file) {
  ...
  const { error } = await supabase.storage.from(STAFF_CHAT_BUCKET).upload(path, file, {
    cacheControl: '3600',
    upsert: false,
  })
  if (error) throw error
  return { path, name: safeName, mime: file.type || 'application/octet-stream' }
}
