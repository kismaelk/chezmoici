...
async function withSignedStaffChatAttachment(row) {
  const path = staffChatAttachmentPath(row.attachment_url)
  if (!path) return row
  ...
  return {
    ...row,
    attachment_url: error ? null : (data?.signedUrl ?? null),
  }
}
...
export async function fetchStaffDiscussionMessages(limit = 200) {
  ...
  const rows = await Promise.all(
    (data || []).map((m) =>
      withSignedStaffChatAttachment(mapStaffDiscussionRow(m, reactionRows, user?.id))
    )
  )
  rows.reverse()
  return rows
