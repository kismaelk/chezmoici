...
export async function getStaffFromRequest(request) {
  ...
  const { data: profil, error: profilErr } = await supabaseUser
    .from('profiles')
    .select('id, is_admin, admin_role, email, nom, account_status, account_suspended_until')
    .eq('id', user.id)
    .single()
  ...
  if (profil.account_status === 'banned' || estCompteSuspenduJusqua(profil.account_suspended_until)) {
    return { error: 'Compte suspendu', status: 403 }
  }
