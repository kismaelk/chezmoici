...
function profilBloqueStaff(profil) {
  if (!profil) return true
  if (profil.account_status === 'banned' || profil.account_status === 'suspended') return true
...
export function resolveStaffRole(profil, email) {
  if (!profil?.is_admin || profilBloqueStaff(profil)) return null
