...
/**
 * Un compte staff banni ou suspendu ne doit plus ouvrir de session admin.
 */
export function isBlockedStaffProfile(profil) {
  if (!profil) return false
  if (profil.account_status === 'banned' || profil.account_status === 'suspended') {
    return true
  }
  if (!profil.account_suspended_until) return false
  const t = new Date(profil.account_suspended_until).getTime()
...
export function resolveStaffRole(profil, email) {
  if (!profil?.is_admin || isBlockedStaffProfile(profil)) return null
