...
export function isStaffProfileRestricted(profil, nowMs = Date.now()) {
  if (!profil) return true
  if (profil.account_status === 'banned' || profil.account_status === 'suspended') {
    return true
  }
  if (profil.account_suspended_until == null || profil.account_suspended_until === '') {
    return false
  }
  const suspendedUntil = new Date(profil.account_suspended_until).getTime()
  return !Number.isNaN(suspendedUntil) && suspendedUntil > nowMs
}
...
export function resolveStaffRole(profil, email) {
  if (!profil?.is_admin || isStaffProfileRestricted(profil)) return null
