...
async function rejeterSiProfilBloque(user) {
  if (!user?.id) return
  ...
  if (estProfilCompteBloque(profil)) {
    await supabase.auth.signOut()
    throw new ErreurCompteSuspendu(profil.account_suspended_until)
  }
}
...
export async function connecterAvecEmail(email, motDePasse) {
  ...
  if (error) throw error
  await rejeterSiProfilBloque(data.user)
  return normalizeUser(data.user)
}
...
export async function verifierCodeConnexionSMS(telephone, code) {
  ...
  if (error) throw error
  if (data?.user) await rejeterSiProfilBloque(data.user)
