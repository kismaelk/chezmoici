...
/**
 * Export Excel multi-feuilles : body.sheets = [{ sheetName, columns, rows }]
 * @param {unknown} body
 */
export function parseExportSheetsPayload(body) {
  if (!body || typeof body !== 'object') return { error: 'Corps JSON invalide' }
  const sheets = body.sheets
  if (!Array.isArray(sheets) || sheets.length === 0) return { error: 'sheets requis' }
  if (sheets.length > 12) return { error: 'Maximum 12 feuilles' }
  const parsed = []
  let totalRows = 0
  for (const sh of sheets) {
    const p = parseExportTablePayload(sh)
    if (p.error) return { error: p.error }
    totalRows += p.rows.length
    if (totalRows > MAX_EXPORT_ROWS) {
      return { error: `Plafond ${MAX_EXPORT_ROWS} lignes au total` }
    }
    const sheetRaw = typeof sh.sheetName === 'string' ? sh.sheetName.trim() : ''
    parsed.push({
      sheetName: (sheetRaw || 'Feuille').slice(0, 31),
      columns: p.columns,
      rows: p.rows,
    })
  }
